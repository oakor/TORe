...
# debug
def save_debug_info(data, filename="tmp.json"):
    """
    debug
    ...
    try:
        for op, explanation in zip(operations, explanations):
            if op.startswith('f_add_knowledge_column'):
                ...
                print("rows: ", rows)
                rows = [int(row.strip().strip('row').strip()) for row in rows]
...
def generate_qa_prompts_batch(original_tables: List[List[List[Any]]], 
    ...
    """
    # is_multi_table
    print("generate_qa_prompts_batchis_multi_table: ", is_multi_table)
...
def generate_qa_pairs_batch(tables: List[List[List[Any]]], chains: List[List[str]], llm: LLM, sampling_params: SamplingParams, table_ids: List[str] = None, is_multi_table: bool = False) -> List[Dict[str, Any]]:
    """
    ...
    """
    # is_multi_table
    print("generate_qa_pairs_batchis_multi_table: ", is_multi_table)
    ...
    for table in tables:
        if is_multi_table and isinstance(table, dict):
            # 
            processed_tables.append(table["table_text"])
            table_captions.append(table.get("table_caption"))
            foreign_keys.append(table.get("foreign_key"))
        else:
            # 
            processed_tables.append(table)
            table_captions.append(None)
            foreign_keys.append(None)
    
    # 1. 
    chain_params = generate_chain_params_batch(processed_tables, chains, llm, sampling_params, is_multi_table, table_captions, foreign_keys)
    
    # 2. 
    processed_results = process_tables_with_params(processed_tables, chains, chain_params, llm, sampling_params, is_multi_table, table_captions, foreign_keys)
    
    # 3. 
    print(f"generate_qa_pairs_batchgenerate_qa_prompts_batchis_multi_table: {is_multi_table}")
    qa_prompts = generate_qa_prompts_batch(original_tables=processed_tables, processed_results=processed_results, chains=chains, is_multi_table=is_multi_table, table_captions=table_captions, foreign_keys=foreign_keys)
...
def main():
    ...
    # 
    llm = LLM(model=model_path, tensor_parallel_size=2, max_model_len=10000)
    print("Loading...")    
    ...
    for task_id in range(8, 9):
        global task_ID
        task_ID = task_id
        multi_table_identifier = is_multi_table[task_id]
        print(f"task_id={task_id}，multi_table_identifier={multi_table_identifier}")
        train_file = f"{train_file_root}/task_{task_id}/train.json"
        processed_file = f"{processed_file_root}/processed_chains_{task_id}.json"
        output_file = f"{output_file_root}/generated_qa_pairs_{task_id}.json"
        

        # 
        print(f"convert_to_ftct_formatmulti_table_identifier: {multi_table_identifier}")
        tables = convert_to_ftct_format(train_file, multi_table_identifier)
        train_data = []
        for item in tables:
            if multi_table_identifier:
                # 
                train_item = {
                    "id": item["id"],  # ID
        ...
        while len(outputs) < 500:
            ...
            # 
            if multi_table_identifier:
                for i in range(len(sub_batch_tables)):
                    if isinstance(sub_batch_tables[i], list) and isinstance(sub_batch_tables[i][0], list) and isinstance(sub_batch_tables[i][0][0], list):
                        # 
                        sub_batch_tables[i] = {
                            "table_text": sub_batch_tables[i],
                            "table_caption": sub_batch_table_captions[i],
                            "foreign_key": sub_batch_foreign_keys[i]
                        }
            
            # 
            print(f"maingenerate_qa_pairs_batchmulti_table_identifier: {multi_table_identifier}")
            batch_results = generate_qa_pairs_batch(
                ...
                is_multi_table=multi_table_identifier  # 
            )
