...
for task_id in range(9):
    random.seed(task_id)
    # Define paths
    source_file = f'{source_file_root}/qa_with_reasoning_{task_id}_gaps.json'
    if task_id == 0:
        source_file = f'{source_file_root}/qa_with_reasoning_0.json'
    target_file = f'{target_file_root}/train.json'
    output_file = f'{output_file_root}/outdomain_train_{task_id}_100.json'
    ...
    try:
        source_data = random.sample(temp_data, 100)
    except:
